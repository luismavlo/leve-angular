...
export class SaleService {
  ...
  getSaleById(id: string): Observable<DataSale> {
    return this.http.get<DataSale>(`http://localhost:3000/api/v1/sales/${id}`, {
  ...
  getSales(offset: number, limit: number = 5): Observable<SaleResp> {
    ...
    return this.http.get<SaleResp>(`http://localhost:3000/api/v1/sales`, {
      headers: tokenHeader.headers,
      params,
    });
  }
  ...
  getSaleByTerm(
    ...
    return this.http.get<SaleResp>(`http://localhost:3000/api/v1/sales/${type}/${term}`, {
      headers: tokenHeader.headers,
      params,
    });
  }
  ...
  createSale(sale: Sale): Observable<any> {
    return this.http.post<any>(`http://localhost:3000/api/v1/sales`, sale, {
      headers: tokenHeader.headers,
    });
  }
  ...
  updateSale(sale: Sale, id: string): Observable<any> {
    return this.http.patch<any>(`http://localhost:3000/api/v1/sales/${id}`, sale, {
      headers: tokenHeader.headers,
    });
  }
  ...
  deleteSale(id: string): Observable<any> {
    return this.http.delete<any>(`http://localhost:3000/api/v1/sales/${id}`, {
      headers: tokenHeader.headers,
    });
  }
}
