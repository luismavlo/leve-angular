...
export class ExpensesService {
  ...
  getAllExpenses(offset: number): Observable<ExpenseResp> {
    ...
    return this.http.get<ExpenseResp>(`http://localhost:3000/api/v1/expenses`, {
  ...
  getAllExpensesByType(
    ...
    return this.http.get<ExpenseResp>(`http://localhost:3000/api/v1/expenses/type/${term}`, {
      headers: tokenHeader.headers,
      params,
    });
  }
  ...
  getExpensesById(id: string): Observable<DataExpense> {
    return this.http.get<DataExpense>(`http://localhost:3000/api/v1/expenses/${id}`, {
      headers: tokenHeader.headers,
    });
  }
  ...
  createExpense(expense: Expense): Observable<DataExpense> {
    return this.http.post<DataExpense>(`http://localhost:3000/api/v1/expenses`, expense, {
      headers: tokenHeader.headers,
    });
  }
  ...
  updateExpense(expense: Expense, id: string): Observable<DataExpense> {
    return this.http.patch<DataExpense>(`http://localhost:3000/api/v1/expenses/${id}`, expense, {
      headers: tokenHeader.headers,
    });
  }
  ...
  deleteExpense(id: string): Observable<any> {
    return this.http.delete<any>(`http://localhost:3000/api/v1/expenses/${id}`, {
      headers: tokenHeader.headers,
    });
  }
  ...
  createExpensePerLot(lotId: string, expenseId: string) {
    return this.http.post(
      `http://localhost:3000/api/v1/expenses-per-lots`,
      {
        lotId,
        expenseId,
      },
